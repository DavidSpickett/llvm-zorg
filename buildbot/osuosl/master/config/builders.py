...
# Cross builders.
    ...
    'factory' : UnifiedTreeBuilder.getCmakeExBuildFactory(
                    ...
                    cmake_definitions = {
                        ...
                        "TOOLCHAIN_TARGET_SYSROOTFS"    : util.Interpolate("%(prop:sysroots)s/armv7-linux-gnueabihf"),
                        "ZLIB_ROOT"                     : util.Interpolate("%(prop:sysroots)s/zlib-win32"),
...
# Clang builders.
    ...
    'factory' : UnifiedTreeBuilder.getCmakeExBuildFactory(
                    depends_on_projects=['llvm', 'clang'],
                    checks=['check-llvm', 'check-clang'],
                    clean=True,
                    jobs=8,
                    cmake_definitions={
                        'CMAKE_BUILD_TYPE'               : 'Release',
                        'LLVM_COMPILER_CHECKED'          : '1',
                        'LLVM_ENABLE_PROJECTS'           : 'clang',
                        'HAVE_CXX_ATOMICS_WITHOUT_LIB'   : 'ON',
                        'HAVE_CXX_ATOMICS64_WITHOUT_LIB' : 'ON',
...
# OpenMP AMDGPU Builders
    ...
    'factory' : ScriptedBuilder.getScriptedBuildFactory(
                        "offload/ci/openmp-offload-amdgpu-runtime.py",
